Feature: Transaction
  ...
  Scenario: "Transaction" report Filters option functions properly
    ...
    And the user sets the Filter to start with "107" of the Transaction Report
    And the user clicks on the Apply button of the Transaction Report
    And the user extracts all the report values in the grid
    And the user verifies that all the Customer Name values on the Transaction Report start with "107"
